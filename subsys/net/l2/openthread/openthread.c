...
static void ot_l2_state_changed_handler(uint32_t flags, struct otInstance *instance,
					void *user_data)
{
	...
	if (flags & OT_CHANGED_NAT64_TRANSLATOR_STATE) {
		NET_DBG("Nat64 translator state changed to %x",
			otNat64GetTranslatorState(instance));
	}
...
static int openthread_l2_init(struct net_if *iface)
{
	...
	struct openthread_state_changed_callback ot_l2_state_changed_cb = {
		.openthread_state_changed_cb = ot_l2_state_changed_handler,
		.user_data = (void *)ot_l2_context,
	};

	ll_addr = net_if_get_link_addr(iface);

	/* Check whether the OpenThread and its instance were properly initialized by the OpenThread
	 * module
	 */
	__ASSERT(openthread_init(ot_receive_handler, (void *)ot_l2_context),
		 "OT instance is not initialized");
	...
	/* To keep backward compatibility use the additional state change callback list from the ot
	 * l2 context and register the callback to the openthread module.
	 */
	sys_slist_init(&ot_l2_context->state_change_cbs);
	openthread_state_change_callback_register(&ot_l2_state_changed_cb);
